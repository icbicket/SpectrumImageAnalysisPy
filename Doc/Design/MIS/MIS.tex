...
All modules within \progname{} are accessible by the user from terminal commands. Some modules will interface with each other, but the workflow of \progname is driven by the user.
...
\begin{itemize}
    ...
    \item transition: N/A
    \item output: h5 file containing spectrum data, image data, or spectrum image data (including any metadata), written to filesystem
    \item exception:
    \begin{center}
        \begin{tabular}{p{3cm} p{12cm}}
            \toprule[0.15em]
...
\subsubsection{State Variables}
\begin{itemize}
    \item slice: interval of \hyperref[Mod:Spectrum]{Spectrum} between $[k_1, k_2]$
\end{itemize}
