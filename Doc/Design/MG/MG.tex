...
\begin{description}
	...
	\item[\refstepcounter{acnum} \actheacnum \label{acCSVheadings}:] The headings or data format of saved spectrum csv files.
	\item[\refstepcounter{acnum} \actheacnum \label{acDM3datatype}:] The data type inside dm3 files.
	\item[\refstepcounter{acnum} \actheacnum \label{acDM3labels}:] The locations/labels of data and metadata inside dm3 files.
	\item[\refstepcounter{acnum} \actheacnum \label{acDM4support}:] Expanding support to dm4 files.
	\item[\refstepcounter{acnum} \actheacnum \label{acH5datatype}:] The data type inside h5 files.
	\item[\refstepcounter{acnum} \actheacnum \label{acH5labels}:] The locations/labels of data and metadata inside h5 files.
	\item[\refstepcounter{acnum} \actheacnum \label{acNewDataProcessing}:] Adding new data processing algorithms
	\item[\refstepcounter{acnum} \actheacnum \label{acRLoptimize}:] Functions used for RL deconvolution (optimizations for reduced error or speed).
	\item[\refstepcounter{acnum} \actheacnum \label{acNorm}:] Optimizations on normalization algorithm
	\item[\refstepcounter{acnum} \actheacnum \label{acGain}:] Optimizations on gain correction algorithm
	\item[\refstepcounter{acnum} \actheacnum \label{acBkgnd}:] Optimizations on background correction algorithm
	\item[\refstepcounter{acnum} \actheacnum \label{ac1DplotFormat}:] Format of 1D spectrum plot
	\item[\refstepcounter{acnum} \actheacnum \label{ac2DplotFormat}:] Format of 2D image plot
	\item[\refstepcounter{acnum} \actheacnum \label{ac3DPlotLayout}:] Layout of 3D spectrum image plotting display
	\item[\refstepcounter{acnum} \actheacnum \label{ac3DPlotOptimize}:] Optimizations for faster response of spectrum image plotting display
	\item[\refstepcounter{acnum} \actheacnum \label{ac3DPlotCommands}:] Changes to the interactive commands in the spectrum image plotting display
	\item[\refstepcounter{acnum} \actheacnum \label{acSpectrumUnits}:] Units or label on the spectral axis
	\item[\refstepcounter{acnum} \actheacnum \label{ac1DSlice}:] Method used to slice spectra
	\item[\refstepcounter{acnum} \actheacnum \label{ac2DMask}:] Method of masking 2D images
	\item[\refstepcounter{acnum} \actheacnum \label{ac3DMask}:] Method of masking 3D images
	\item[\refstepcounter{acnum} \actheacnum \label{ac4Dsupport}:] Adding new modules for image stacks (4D datasets)
	\item[\refstepcounter{acnum} \actheacnum \label{acImageExportFormat}:] New file format for exported image files (\textit{e.g.}, change to tiff)
	\item[\refstepcounter{acnum} \actheacnum \label{acSIExportFormat}:] New file format for exporting spectrum image files
	\item[\refstepcounter{acnum} \actheacnum \label{acSIExportH5Format}:] Format changes for exporting spectrum image files to h5
	\item[\refstepcounter{acnum} \actheacnum \label{acSIExportRPLFormat}:] Format changes for exporting spectrum image files to rpl.
\end{description}
...
\begin{description}
	\item[\refstepcounter{ucnum} \uctheucnum \label{ucIO}:] Input/Output hardware devices (a keyboard and mouse for input, a monitor screen for output)
	\item[\refstepcounter{ucnum} \uctheucnum \label{ucHardware}:] The nature of the hardware used to run the software (a laptop or desktop computer containing sufficient memory and computing power to perform the operations required)
	\item[\refstepcounter{ucnum} \uctheucnum \label{ucInput}:] There will always be
	  a source of input data external to the software.
	\item[\refstepcounter{ucnum} \uctheucnum \label{ucArray}:]  The use of arrays for holding data.
	\item[\refstepcounter{ucnum} \uctheucnum \label{ucPlottingLibrary}:]  Plotting library used
	\item[\refstepcounter{ucnum} \uctheucnum \label{ucArrayLibrary}:]  Array structure library used
	\item[\refstepcounter{ucnum} \uctheucnum \label{uc1DdataFormat}:]  Format of 1D data (y-axis and x-axis data)
	\item[\refstepcounter{ucnum} \uctheucnum \label{uc2DdataFormat}:]  Format of 2D data ($(x,y)$ data array)
	\item[\refstepcounter{ucnum} \uctheucnum \label{uc3DdataFormat}:]  Format of 3D data ($(x,y,E)$ data array)
	\item[\refstepcounter{ucnum} \uctheucnum \label{uc3DSIAxisOrder}:] The order of the spatial and spectral axes in a spectrum image
	\item[\refstepcounter{ucnum} \uctheucnum \label{ucExDataType}:] The data type to export
\end{description}
