...
class CLSet(object):
	...
	def __init__(self, samplefile, darkfile, substratefile):
		...
		self.sample_dark = CLSpectrumData.CLDataSet(SI = self.SpectrumSubtraction(self.SampleSI, self.darkSI), SEM = self.SampleSI.SEM.Img, survey = self.SampleSI.survey.Img)
		self.substrate_dark = CLSpectrumData.CLDataSet(SI = self.SpectrumSubtraction(self.substrateSI, self.darkSI), SEM = self.SampleSI.SEM.Img, survey = self.SampleSI.survey.Img)
		"""Perform substrate correction on sample data"""
		self.sample_dark_substrate = CLSpectrumData.CLDataSet(SI = self.SpectrumSubtraction(self.sample_dark, self.substrate_dark), SEM = self.SampleSI.SEM.Img, survey = self.SampleSI.survey.Img)
...
class PolarimetrySet(object):
	...
	def __init__(self, PolSetData):
		...
		self.offsets = AlignLib(PolSetData)
